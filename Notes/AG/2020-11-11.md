...
1. $\ssets \homotopic \Top$: this is not an equivalence of categories, but rather they equivalent homotopy theories (theory up to weak equivalence).
  ...
  Here there is an $n\dash$simplex on the LHS, 
  \[
...
If you make a function $\Delta^n \mapstofrom \Delta^n_{\text{Top}}$, by Yoneda, the presheaf category $\Presh(\Delta) \da \Fun(\Delta^\op, \set)$ is generated by representable objects 
