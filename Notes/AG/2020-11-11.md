...
\end{tikzcd}
We make this into a chain complex by setting the differential $M_1 \mapsvia{\sum(-1)^i} M_0$.
...
Defined as $s\mathrm{CAlg}_k = \Fun(\Delta\op,\mathrm{CAlg}_k)$, where $k$ is some commutative ring.
This was studied by Quillen, and an impetus for model categories.
Models give a notion of weak equivalence and a "right way" of computing: for the usual derived category, this is taking resolutions.
:::
